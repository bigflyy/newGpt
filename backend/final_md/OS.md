...
**+(Плюсы монитора)** : 1. Избавляет программиста от деталей реализации работы с разделяемыми данными; 
...
Имена **ПИ** --**И**-> Идентификаторы **ПА** --**П**-> Ячейки **ФА** --**З**-> Значения **З**             
...
**Плюсы СЕГМЕНТНОЙ ОРГАНИЗАЦИИ ПРОГРАММ**
а) Сегменты могут размещаться в несмежных областях памяти. 
...
**Минусы СЕГМЕНТНОЙ ОРГАНИЗАЦИИ ПРОГРАММ**
...
Б) Высокие накладные расходы на сопровождение миграции сегментов между ОЗУ
и внешней памятью. Профилактика: страничная организация памяти!

B) Два обращения к ОЗУ (первое – к таблице за адресом сегмента, второе – к адресуемому объекту внутри сегмента). Существенно снижает быстродействие вычислителя. Профилактика: полностью ассоциативное кэширование адресных ссылок на сегменты (емкость кэша адресных ссылок TLB обычно несколько сотен строк), реализуется с помощью отображения: 
...
**----- Start of picture text -----**<br>
Схема Иллюстрация концепции сегментации памяти и трансляции адресов
...
**Минусы** А) Два обращения к ОЗУ. Профилактика – кэширование отображения логических адресов страниц в физические (решается, так же как и в случае сегментной организации программ). 
...
**Плюсы Сегментно-страничного способа распределения памяти**
...
**Минусы Сегментно-страничного способа распределения памяти**
...
**Система Управления Файлами (СУФ)** имеет многоуровневую структуру: 
...
Анализ системы – Формализация (идеализация) – Решение – Интерпретация результатов_ 
...
Центром NASA сформулированы основополагающие требования, которым должны удовлетворять тестовые методики оценки производительности супер вычислителей: 
...
$$
\begin{aligned}
\mathrm{SPECintXXXX} &= \left( \prod_{i=1}^{n} \mathrm{SPECint ratio}(i) \right)^{1/n}, \quad 
\mathrm{SPECfpXXXX} = \left( \prod_{j=1}^{m} \mathrm{SPECfp ratio}(j) \right)^{1/m}, \\
\mathrm{SPECmarkXXXX} &= \left( \prod_{i=1}^{n} \mathrm{SPECint ratio}(i) \times \prod_{j=1}^{m} \mathrm{SPECfp ratio}(j) \right)^{1/n+m}.
\end{aligned}
$$
...
Области адресуемого пространства должны быть ограждены **стенами с воротами** для обеспечения коммуникационных связей. **Стены** должны быть связаны с механизмом **адресации** , адресные ссылки внутри области не должны требовать обширных проверок, **адресные ссылки на объекты вне данной области** должны реализовываться через механизм – **ворота** . Коммуникационные связи (ворота) должны охраняться механизмом контроля, который разрешал бы доступ только субъектам, имеющим на это право. 

-- Чанк --

Способы возведения стен: **права доступа** , **обеспечение секретности пути к объекту** , **применение методов кодирования** . 

**Права доступа** . При сегментированном пространстве адресов каждый адрес внутри сегмента проверяется на принадлежность диапазону от начального адреса до начального адреса + длина сегмента. Т. о. адресный механизм возводит **Стену** вокруг всех адресов внутри сегмента и **Ворота.** Сами сегменты защищаются через таблицу сегментов при проверке законности обращения (в соответствии со статусом защиты) со стороны процесса, выдавшего ссылку. 
...
**Правило 2.** Домен **х(i)** может скопировать атрибут доступа Аk в элемент **Аk** и **А(х(j),у)** , если атрибут находится в соответствующем элементе **А(х(i),у)** для него установлен атрибут **признак копирования** . 
...
\sh_i = E(o_i + \sh_{i-1}),
\]
где \( o_i \) — \(i\)-й блок открытого текста,  
\( \sh_i \) — \(i\)-й блок шифрованного текста,  
\( E \) — функция шифрования. 

**Дешифрование:** 

\[
o_i = E^{-1}(\sh_i) + \sh_{i-1}.
\] 

При дешифровании дешифруется очередной шифрованный блок складывается по модулю 2 с предыдущим шифрованным блоком ( **по прямой связи** ). 
...
6. Для шифрования блока сообщения необходимо вычислить C = X^E по 
...
Программная реализация крипто алгоритмов типа RSA существенно сложнее и значительно менее производительна, чем реализация классических крипто алгоритмов типа DES. Обусловлено это сложностью реализации модульной арифметики (с высокой разрядностью обрабатываемых чисел). Обычно RSA используют для: 
...
В) Сравнивает **свертку и ПОДПИСЬ после дешифрирования** , при равенстве – документ оригинален, при неравенстве – документ поддельный (изменен злоумышленником). 
...
В **SMP** -вычислителях мультипроцессорную обработку организуют многопроцессорные ОС. 
...
– возможность взаимодействия приложений через общую область памяти. 

К недостаткам следует отнести: 

– дублирование управляющих структур при каждом процессоре, 

– невозможность динамического перераспределения используемой памяти, 
...
**----- Start of picture text -----**<br>
Схема Архитектура многопроцессорной системы с общей системной шиной
Компоненты схемы
...
**----- Start of picture text -----**<br>
Схема Симметричная мультипроцессорная модель (SMP)
...
**----- Start of picture text -----**<br>
Схема Планирование времени в мультипроцессорной системе
...
- минимизирует конкуренцию за списки процессов (поскольку списки для каждого процессора – свои), 
...
**----- Start of picture text -----**<br>
Схема Родственное планирование в мультипроцессорной системе
...
**Логика организации совместно используемой памяти и единого адресного пространства может быть реализована:** 
...
**----- Start of picture text -----**<br>
Схема Модель распределённой памяти совместного пользования
...
– перенос файла целиком эффективнее, чем перенос его по частям. 

Недостатки: 

– необходимость наличия достаточно большого объема памяти на клиенте для хранения файла целиком, 

– накладные расходы при переносе файла целиком, в то время как требуется только его часть, 
...
3. **Модель закачивания-скачивания с автоматической блокировкой** скачанных файлов. Попытки всех остальных клиентов получить уже считанный кем-то файл будут приостановлены до того момента, пока первый клиент не вернет файл. При повышенном спросе на файл сервер может посылать клиенту, удерживающему файл, сообщения об ускорении возврата файла. 
...
2. Запрос передается через коммутатор или другую коммуникационную систему в узел с резидентным для строки блоком памяти. 
3. Из резидентного узла необходимая строка пересылается в узел, в котором произошел промах. 
...
Более совершенным является рассматриваемый ниже алгоритм **DASH** . Каждый блок памяти имеет для каждой строки, резидентной в узле, список узлов, в кэшах которых размещены копии строк. Каждая резидентная строка может находиться в трех **глобальных** состояниях: 
...
В локальных состояниях строки «невозможная к использованию» или «разделяемая» запись в нее начинается с посылки в резидентный для строки узел запроса на захват ее в исключительное использование и приостановки выполнения записи до получения подтверждений о том, что остальные узлы, разделяющие с ним рассматриваемую строку, перевели ее копии в локальное состояние «невозможная к использованию». 
...
| Исходное состояние |                   | Состояние после чтения |                   | Состояние после записи |                   |
|---------------------|-------------------|------------------------|-------------------|------------------------|-------------------|
| Глобальное          | Локальное         | Глобальное             | Локальное         | Глобальное             | Локальное         |
| NC                  |                   | RS или NC при чтении в резидентном узле | S | RM/NC | M |
| RS                  | NU                | RS                     | S                 | RM                     | M                 |
| RS                  | S                 | RS                     | S                 | RM                     | M                 |
| RS                  | M                 | RS                     | S                 | RM                     | M                 |
| RM                  | NU                | RS                     | S                 | RM                     | M                 |
...
Причина этого состоит в высокой сложности применения внутреннего механизма когерентности иерархической памяти узла по сравнению с использованием предусмотренного для работы с внешними устройствами механизма когерентности. 
